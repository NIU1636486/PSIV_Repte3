# -*- coding: utf-8 -*-
"""
Created on Sat Nov 23 18:27:10 2024
...
patientsImgsCropped, patientsMetaCropped = loadCropped(os.listdir(pathDir_servidor), None)

pathDir_servidor = "/fhome/maed/HelicoDataSet/CrossValidation/Cropped"
# Dades per la validació del patch classification
patientsImgsAnnotated, patientsMetaAnnotated = loadAnnotated(os.listdir(pathDir_servidor), None)
...
# LListes per guardar els resultats dels tests
ResultsTestPatchClassification = list()
ResultsTestPatientsDiagnosis = list()
...
####################### TRAIN PATCH CLASSIFICATION ###########################
    ...
    OptimalThresholdsPatchClassification.append([optimalThreshold, fpr, tpr])
...
############################# METRIQUES DE VALIDACIÓ #########################################################    
    ...
    # Guardem els resultats del test de patch classification
    ResultsTestPatchClassification.append(patchClassification)
...
####################### TRAIN PATIENT DIAGNOSIS ###############################
    ...
    optimalTreshold = OptimalThresholdsPatchClassification[fold][0]
    ...
    OptimalThresholdsPatientsDiagnosis.append([optimalDiagnosis, fpr, tpr])
...
############################# METRIQUES DE VALIDACIÓ #########################################################    
    ...
    # Guardem els resultats del test de patient diagnosis
    ResultsTestPatientsDiagnosis.append(patientDiagnosis)

# Guardem les llistes en un diccionari
lists = {"OTPC":OptimalThresholdsPatchClassification, "OTPD":OptimalThresholdsPatientsDiagnosis, "RTPC":ResultsTestPatchClassification, "RTPD":ResultsTestPatientsDiagnosis}

# Creem un arxiu pickle per guardar les dades 
with open('validation.pkl', 'wb') as file:  # 'wb' es para escritura en modo binario
    pickle.dump(lists, file)

print("Llistes guardades correctament.")
