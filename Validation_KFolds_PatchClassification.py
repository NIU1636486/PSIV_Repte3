...
for fold, (train_idx, val_idx) in enumerate(kf.split(X, y, patientsID)):
    ...
    train_meta = [y[i] for i in train_idx]
    test_meta = [patientsMeta[i] for i in val_idx]
    folds.append([train_imgs,test_imgs,train_meta,test_meta])
    
    # Train reconstruction error
    print(f'\nTraining Fold {fold}....')
    for input_image, label in zip(train_imgs,train_meta):
    ...
    for key,value in positive.items():
        print(f"Percentatge de finestres positives pel pacient {key} =",(value/10)*100,"%")
